...
const setFilterRoutes = () => {
  ...
  if (layout === 'classic' && isClassicSplitMenu) {
    state.menuList = delClassicChildren(filterRoutesFun(routesListSate.value.routesList));
    const resData = setSendClassicChildren(route.path);
    mittBus.emit('setSendClassicChildren', resData);
  } else {
    state.menuList = filterRoutesFun(routesListSate.value.routesList);
  }
};
...
const setSendClassicChildren = (path: string) => {
  ...
  filterRoutesFun(routesListSate.value.routesList).map((v: RouteItem, k: number) => {
