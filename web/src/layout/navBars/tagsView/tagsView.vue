    ...
    <el-scrollbar ref="scrollbarRef" @wheel.prevent="onHandleScroll">
      <ul class="layout-navbars-tagsview-ul" :class="setTagsStyle" ref="tagsUlRef">
        ...
        >
          ...
          <span>{{ $t(setTagsViewNameI18n(v)) }}</span>
...
const storesRoutesList = useMenuStore();
...
const singleAddTagsView = (path: string, to?: RouteToFrom) => {
  ...
  state.tagsViewList.forEach((v) => {
    if (
        v.path === isDynamicPath &&
        !isObjectValueEqual(
            to?.meta?.isDynamic ? (v.queryParams ? v.queryParams : null) : v.query ? v.query : null,
            to?.meta?.isDynamic ? (to?.params ? to?.params : null) : to?.query ? to?.query : null
        )
    ) {
...
const addTagsView = (path: string, to?: RouteToFrom) => {
  ...
  nextTick(async () => {
    ...
    if (to?.meta?.isDynamic) item.params = to?.queryParams ? to?.queryParams : route.queryParams;
        ...
        setTimeout(() => {
          if (state.tagsViewList.length === k && getThemeConfig.value.isShareTagsView ? state.routePath === path : state.routeActive === path) {
            ...
            if (arr[arr.length - 1].meta.isDynamic) {
              // 动态路由（xxx/:id/:name"）
              if (k !== arr.length) router.push({name: arr[k].name, queryParams: arr[k].params});
              else router.push({name: arr[arr.length - 1].name, queryParams: arr[arr.length - 1].params});
            } else {
              // 普通路由
            ...
            if (state.tagsViewList.length !== k && getThemeConfig.value.isShareTagsView ? state.routePath === path : state.routeActive === path) {
              if (arr[k].meta.isDynamic) {
                // 动态路由（xxx/:id/:name"）
                router.push({name: arr[k].name, queryParams: arr[k].params});
              } else {
                // 普通路由
...
const openCurrenFullscreen = async (path: string) => {
  ...
  if (item.meta.isDynamic) await router.push({name: item.name, queryParams: item.params});
...
const onCurrentContextmenuClick = async (item: RouteItem) => {
  ...
  switch (item.contextMenuClickId) {
    case 0:
      // 刷新当前
      if (meta.isDynamic) await router.push({name, params: queryParams});
...
const onTagsClick = (v: RouteItem, k: number) => {
  ...
  if (getThemeConfig.value.layout === 'columns') {
    const item: RouteItem = routesListSate.value.routesList.find((r: RouteItem) => r.path.indexOf(`/${v.path.split('/')[1]}`) > -1);
...
const transUrlParams = (v: RouteItem) => {
  let params = v.query && Object.keys(v.query).length > 0 ? v.query : v.queryParams;
  if (!params) return '';
  let path = '';
