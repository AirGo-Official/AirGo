...
const onColumnsAsideMenuMouseleave = async () => {
  ...
  setTimeout(() => {
    if (!routesListSate.value.isColumnsMenuHover && !routesListSate.value.isColumnsNavHover) mittBus.emit('restoreDefault');
  }, 100);
};
// 设置高亮动态位置
...
const setFilterRoutes = () => {
  state.columnsAsideList = filterRoutesFun(routesListSate.value.routesList);
...
watch(
    [() => themeConfig.value.columnsAsideStyle, routesListSate.value.isColumnsMenuHover, routesListSate.value.isColumnsNavHover],
    () => {
      themeConfig.value.columnsAsideStyle === 'columnsRound' ? (state.difference = 3) : (state.difference = 0);
      if (!routesListSate.value.isColumnsMenuHover && !routesListSate.value.isColumnsNavHover) {
