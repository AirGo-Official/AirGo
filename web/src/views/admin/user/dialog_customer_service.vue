...
<!--        <el-table-column prop="user_name" :label="$t('message.adminUser.CustomerService.user_name')" width="120" fixed/>-->
        <el-table-column prop="service_status" :label="$t('message.adminUser.CustomerService.service_status')" width="80">
        ...
        <el-table-column prop="service_start_at" :label="$t('message.adminUser.CustomerService.service_start_at')" width="150">
          <template #default="{row}">
            {{ DateStrToTime(row.service_start_at) }}
          </template>
        </el-table-column>
        <el-table-column prop="service_end_at" :label="$t('message.adminUser.CustomerService.service_end_at')" width="150">
          <template #default="{row}">
        ...
        <div>
          <el-table-column prop="total_bandwidth" :label="$t('message.adminUser.CustomerService.total_bandwidth')" width="60"/>
          <el-table-column prop="node_speed_limit" :label="$t('message.adminUser.CustomerService.node_speed_limit')" width="60"/>
          <el-table-column prop="node_connector" :label="$t('message.adminUser.CustomerService.node_connector')" width="60"/>
          <el-table-column prop="traffic_reset_day" :label="$t('message.adminUser.CustomerService.traffic_reset_day')" width="60"/>
        ...
        <el-table-column :label="$t('message.common.operate')">
          <template #default="{row}">
            <el-button size="small" text type="primary" @click="openDialog(row)">{{$t('message.common.modify')}}</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-drawer>
